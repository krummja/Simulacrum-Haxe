...
class M {
	 ...
	 */
	public static inline var T_INT_BITS =
		#if (neko && neko_v2)
		32;
		#elseif (neko)
		31;
	...
	**/
	...
	public static inline function unit(v: Float, precision = 1): String {
		return M.fabs(v) < 1000 ? Std.string(pretty(v, precision)) : M.fabs(v) < 1000000 ? M.pretty(v / 1000, precision)
			+ "K" : M.pretty(v / 1000000, precision)
			+ "M";
	}
	...
	**/
	public static inline function bezier4(t: Float, p0: Float, p1: Float, p2: Float, p3: Float) {
		t = M.fclamp(t, 0, 1);
		return fastPow3(1 - t) * p0 + 3 * t * fastPow2(1 - t) * p1 + 3 * fastPow2(t) * (1 - t) * p2 + fastPow3(t) * p3;
	}
	...
	**/
	public static inline function bezier3(t: Float, p0: Float, p1: Float, p2: Float) {
		t = M.fclamp(t, 0, 1);
		return fastPow2(1 - t) * p0 + 2 * t * (1 - t) * p1 + fastPow2(t) * p2;
	}
